...
"""별도 미리보기 빌더 — 기존 build_site.py를 '수정 없이' 재사용해
   MP3 음성 출력(심화) 챕터만 단독 HTML(mp3_preview.html)로 렌더한다.
...
from chapter_mp3 import CHAPTER_MP3 as c
...
out = (TEMPLATE.replace("/*NAV*/", "".join(nav)).replace("/*MAIN*/", main)
       .replace("/*NCODE*/", "2").replace("/*NPROMPT*/", "3").replace("/*NCH*/", "1"))
open(os.path.join(HERE, "mp3_preview.html"), "w", encoding="utf-8").write(out)
print("OK → mp3_lesson/mp3_preview.html (%d bytes)" % len(out))
