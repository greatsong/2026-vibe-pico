...
CHAPTER_SOUND = {
  ...
  "sections": [
    {"title": "시작하기 전에 — 준비물과 순서", "items": [
      ...
      {"type": "concept", "items": [
        {"t": "준비물", "d": "피코 2 WH + 그로브 쉴드, <b>INMP441 전방향 마이크 모듈</b>(핀헤더 납땜형), <b>점퍼선(암-수) 6가닥</b>, WS2813 LED(2장에서 쓰던 것), USB 케이블. <b>새로 살 건 마이크와 점퍼선뿐</b>이에요."},
      ...
      {"type": "callout", "kind": "tip", "title": "우리가 쓰는 마이크 — INMP441 전방향 마이크 (I2S 디지털)",
       "html": "이 챕터의 마이크는 <b>INMP441 전방향 MEMS 마이크</b>(핀헤더 납땜형)예요. 소리를 <b>디지털(I2S)</b>로 곧장 보내 줘서 잡음이 적고 음질이 좋습니다. 그로브 케이블이 아니라 <b>점퍼선 6가닥</b>으로 쉴드의 헤더 핀에 연결해요 — 아래 <b>‘하드웨어 준비’</b>의 회로 구성도를 그대로 따라오면 5분이면 끝나요."},
    ]},
    ...
    {"title": "따라하기 ② 내 소리 직접 모으기 (혼자, 콘솔 키)", "items": [
      ...
      {"type": "code", "label": "코드 ③ · 소리 모으기 (콘솔 라벨링)", "lang": "python", "file": "snippets/sound_record_i2s.py"},
      {"type": "callout", "kind": "info", "title": "이렇게 뜨면 성공! (예상 화면)",
    ...
    {"title": "따라하기 ④ k-NN으로 예측 → LED로 표현", "items": [
      ...
      {"type": "callout", "kind": "info", "title": "왜 하필 k-NN? — 실제 데이터로 확인했어요",
       "html": "공개 음성 데이터(모음·음소 분류)로 여러 알고리즘을 직접 견줘봤더니, <b>피코에 넣을 수 있는 것 중 k-NN이 가장 정확</b>했어요(소리가 다양해질수록 격차가 더 커집니다)."},
    ...
    {"title": "★ 다양하게 실험해보기 — 무엇이든 가르쳐 보세요", "items": [
      ...
      {"type": "callout", "kind": "info", "title": "발걸음·문소리가 잘 안 잡히면? — 그게 정상이고, 그게 핵심",
       "html": "발걸음은 <b>소리가 아주 작고</b>, 사람마다 비슷해요. 잘 안 맞는 건 버그가 아니라 <b>‘특징 안에 신호가 부족하다’</b>는 ML의 정직한 한계랍니다. 해결의 방향은 둘 — ① <b>마이크를 더 가까이</b>(바닥/문 근처), ② <b>특징을 더 추가</b>(아래)."},
    ...
    {"title": "심화 — FFT 주파수밴드로 음색까지 (성능 끌어올리기)", "items": [
      {"type": "text", "html": "예측이 잘 돼야 재밌죠. 더 어려운 소리(휘파람 vs 쉬익, 모음 ‘아’ vs ‘이’처럼 <b>음색만 다른</b> 소리)까지 가르려면 특징을 더 풍부하게 — <b>8개 주파수 대역의 에너지</b>를 씁니다. 음성비서가 쓰는 방식의 축소판이에요. (ulab 없이 Goertzel로 계산해요.)"},
      ...
      {"type": "code", "label": "코드 ⑤ · FFT 주파수밴드 + k-NN (심화)", "lang": "python", "file": "snippets/sound_fft_i2s.py", "fold": True},
      {"type": "callout", "kind": "key", "title": "특징을 늘릴수록…",
       "html": "특징 2개(0.73) → 3개(0.96) → FFT 8밴드는 <b>음색이 비슷한 소리</b>까지 잡아냅니다. INMP441의 깨끗한 16kHz 디지털 소리 덕에 피코에서도 잘 돌아가요. ‘성능 = 좋은 특징’임을 몸으로 확인하는 코너."},
    ]},
    {"title": "자주 하는 실수", "items": [
      {"type": "mistakes", "items": [
        ...
        {"sym": "비슷한 두 소리를 헷갈림", "cause": "특징 안에 구분 신호가 부족(버그 아님).", "fix": "<b>특징 추가</b>(FFT 심화 코너)로 특징을 늘리거나, 마이크를 소리에 <b>더 가까이</b> 두세요."},
      ]},
    ]},
    ...
    {"title": "수업/학습 시나리오", "items": [
      {"type": "text", "html": "한 흐름(자기주도 학습 또는 한두 차시)과 ‘와우 순간’입니다. 모든 무게는 <b>장면 4 — 안 가르친 소리를 맞히는 0.5초</b>에 싣습니다."},
      ...
      {"type": "callout", "kind": "info", "title": "정직한 한 방 (선택)",
       "html": "“쉬운 소리는 90% 넘게 맞히는데, 발걸음은 왜 어려울까?” → 특징을 더하거나(들쭉날쭉·FFT 밴드), 마이크를 소리 가까이. <b>‘특징 안에 신호가 있어야 ML이 된다’</b>를 남기며 마무리."},
    ]},
  ],
}
