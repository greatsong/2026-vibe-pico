...
# ----------------------------------------------------------------- CH2
{
  ...
  "sections": [
    {"title": "배선 — LED 연결하기", "items": [
      {"type": "text", "html": "코드를 넣기 전에, LED 바를 피코(그로브 쉴드)에 먼저 연결해요. LED는 <b>그로브 케이블</b> 하나로 톡 꽂으면 끝이라 아주 간단합니다. (피코·쉴드까지 포함한 전체 조립 그림은 <a class=\"ilink\" href=\"ch0.html#ch0-2\">0장 · 하드웨어 조립</a>을 참고하세요.)"},
      {"type": "callout", "kind": "key", "title": "WS2813 LED 바 → 그로브 D16 포트",
       "html": "LED 바는 <b>그로브 케이블</b>(4핀 커넥터) 하나로 잇는 정품 그로브 모듈이에요. 쉴드의 <b>D16</b> 포트에 꽂으면, 코드에서 <b>16번 핀(GP16)</b>으로 제어됩니다.<br><br><code>np = NeoPixel(Pin(16), NUM, timing=(280,515,515,745))</code>"},
      {"type": "steps", "items": [
        {"t": "그로브 쉴드가 피코에 꽂혀 있는지 확인", "d": "피코가 쉴드에 끝까지 눌러 끼워졌는지 봅니다. (USB 단자가 쉴드 바깥쪽을 향하도록)"},
        {"t": "LED 바에 그로브 케이블 연결", "d": "LED 바 쪽 그로브 소켓에 케이블 한쪽을 딸깍 꽂습니다. 방향이 정해져 있어 한쪽으로만 들어가요."},
        {"t": "반대쪽을 쉴드 D16 포트에", "d": "케이블 반대쪽을 쉴드의 <b>D16</b>(=GP16, 디지털) 포트에 꽂습니다. 코드의 <code>Pin(16)</code>과 같은 번호예요."},
      ]},
      {"type": "callout", "kind": "warn", "title": "A0(아날로그)에 꽂지 마세요",
       "html": "LED는 <b>디지털 D16</b>에 연결합니다. 옆의 <b>A0</b>(아날로그, 3장 가스센서용)에 잘못 꽂으면 LED가 안 켜져요. 포트 글자가 <b>D로 시작(D16)</b>하는지 다시 확인하세요."},
    ]},
...
"라즈베리파이 피코 2 W에서 돌아가는 마이크로파이썬(MicroPython) 코드를 만들어줘. 날씨 데이터가 어떻게 생겼는지 먼저 확인하려는 거야.\n- 무료 날씨 서비스 Open-Meteo(키 필요 없음)에서 우리 지역의 오늘 시간별 강수확률을 받아와.\n- 아침 6시부터 밤 11시(23시)까지를 'N시 강수확률: M%' 형식으로 셸에 출력해줘.\n- 외부 라이브러리는 설치하지 말고, 피코에 기본 내장된 인터넷 접속 기능(socket, ssl)만 써. 보안 인증서 확인은 건너뛰어도 돼.\n- 와이파이 정보는 반드시 from wifi_config import WIFI_SSID, WIFI_PASSWORD 로 불러오고(코드에 직접 쓰지 마), 위도·경도는 코드 맨 위에 LAT=37.5665, LON=126.9780 변수로 둬서 내가 바꿀 수 있게 해줘.\n- 복사해서 바로 돌아가는 코드 전체를 한 번에 줘."},
      ...
      {"type": "prompt", "label": "① AI에게 이렇게 설명하세요 (그대로 복사)", "text":
"라즈베리파이 피코 2 W에서 돌아가는 마이크로파이썬(MicroPython) 코드를 만들어줘. 하루의 비 예보를 LED 색으로 보여주는 '날씨 시계'야.\n- 무료 날씨 서비스 Open-Meteo(키 필요 없음)에서 오늘 시간별 강수확률을 받아와. 인터넷 접속은 피코 기본 내장 기능(socket, ssl)만 쓰고 추가 설치는 하지 마.\n- 와이파이 정보는 반드시 from wifi_config import WIFI_SSID, WIFI_PASSWORD 로 불러오고, 코드에 직접 쓰지 마.\n- 아침 6시~밤 11시(23시, 총 18시간)를 열 칸짜리 LED 바에 고르게 나눠, 각 시각의 강수확률을 색으로 표시해줘. 예: 맑음=초록, 흐림=노랑, 비 가능=파랑, 비 확실=보라. 구간별 색은 코드 위에서 바꿀 수 있게.\n- LED 바(WS2813)는 16번 핀에 연결했어. 만들 때 타이밍 값 네 개(280, 515, 515, 745)를 꼭 지정해줘. 없으면 색이 깨져. 밝기는 낮게(채널당 최대 60 정도).\n- 날씨는 10분에 한 번만 새로 받아와줘.\n- 위도·경도(LAT, LON)와 타이밍 값은 코드 맨 위에 두고, 복사해서 바로 돌아가는 완결형 main.py 전체를 한 번에 줘."},
...
"function doGet(e) {\n  const sh = SpreadsheetApp.getActiveSpreadsheet().getSheets()[0];\n  sh.appendRow([new Date(), Number(e.parameter.value)]);   // [시각, 값] 한 줄 추가\n  return ContentService.createTextOutput(\"OK\");\n}"},
    ...
    {"title": "2단계 · 피코 연결 — 1분마다 자동 기록", "items": [
      ...
      {"type": "prompt", "label": "① AI에게 이렇게 설명하세요 (그대로 복사)", "text":
"라즈베리파이 피코 2 W에서 돌아가는 마이크로파이썬(MicroPython) 코드를 만들어줘. 센서값을 구글 시트에 자동 기록하는 장치야.\n- 가스센서(MQ-2)를 아날로그 입력인 26번 핀으로 읽어, 60초마다 구글 시트에 한 줄씩 기록하고 싶어.\n- 값이 출렁이지 않게 여러 번 읽어 평균을 낸 값을 보내줘.\n- 기록 방법: 구글 앱스 스크립트(Apps Script)로 배포한 웹 앱 주소('/exec'로 끝나는 전체 주소) 뒤에 '?value=측정값'을 붙여 요청을 보내는 방식이야.\n- 외부 라이브러리는 설치하지 말고 피코 기본 내장 기능(socket, ssl)만 써. 보안 인증서 확인은 건너뛰어도 되고, 구글이 다른 주소로 한 번 건너뛰게 하면(리다이렉트) 그것까지 따라가서 마무리해줘. 요청이 성공하든 실패하든 연결(소켓)은 try/finally로 항상 닫아줘 — 60초마다 밤새 반복되니 안 닫으면 메모리가 새서 멈출 수 있어.\n- 와이파이 정보는 반드시 from wifi_config import WIFI_SSID, WIFI_PASSWORD 로 불러오고(코드에 직접 쓰지 마), 웹 앱 주소는 코드 맨 위에 WEB_APP_URL 변수로(꼭 이 이름으로) 두고, 보내는 주기는 INTERVAL 변수로 둬.\n- 복사해서 바로 돌아가는 완결형 main.py 전체를 한 번에 줘."},
...
"[맥락] 내 피코는 지금 ____ 를 하고 있어. (예: 오늘 강수확률을 열 칸 LED에 색으로 표시)\n[하드웨어] 열 칸짜리 LED 바(WS2813)는 16번 핀, 만들 때 타이밍 값 네 개(280, 515, 515, 745) 지정 필요 / 가스센서(MQ-2)는 아날로그 26번 핀 / 와이파이는 from wifi_config import WIFI_SSID, WIFI_PASSWORD 로 불러옴(코드에 직접 쓰지 않기) / 구글 시트에 보낸다면 주소는 WEB_APP_URL 변수 이름 그대로 유지.\n[작업] 여기에 ____ 기능을 더해줘. (예: 강수확률이 60%를 넘으면 LED 깜빡이기)\n[제약] 외부 라이브러리는 최소로, 데이터 갱신은 ____초마다.\n[출력] 복사해서 바로 돌아가는 완결형 main.py 전체를 한 번에 줘.\n받은 코드에서 핀 번호와 타이밍 값이 내 것과 같은지 확인할게."},
