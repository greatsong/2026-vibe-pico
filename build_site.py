  ...
  "sections": [
    {"title": "핵심 개념", "items": [
      ...
      {"type": "teacher", "kind": "theory", "title": "이론 심화 — MQ-2 값이 ppm이 아닌 이유",
       "html": "MQ-2는 내부 저항이 가스 농도에 따라 변하는 <b>반도체식 센서</b>라, 출력 전압이 온도·습도·예열 상태·개체 차이에 함께 흔들립니다. 정확한 ppm을 얻으려면 기준 가스로 개체별 보정 곡선을 만들어야 해서 수업에서는 사실상 불가능하죠. 그래서 이 교재는 처음부터 <b>‘평소보다 높다/낮다’는 상대 지표</b>로만 씁니다. “이 값이 몇 ppm이에요?” 질문이 나오면 “그 질문이 정확히 핵심”이라고 받아 주고, 절대값이 아니라 <b>변화를 읽는 도구</b>라고 정리해 주세요."},
...
# ----------------------------------------------------------------- CH6
{
  "id": "ch6", "num": "06", "title": "자유 프로젝트", "accent": "#8B5CF6",
...
"function doGet(e) {\n  const sh = SpreadsheetApp.getActiveSpreadsheet().getSheets()[0];\n  sh.appendRow([new Date(), e.parameter.value]);   // [시각, 값] 한 줄 추가\n  return ContentService.createTextOutput(\"OK\");\n}"},
      ...
      {"type": "teacher", "kind": "err", "title": "예상 오류 — 시트가 안 쌓이면 십중팔구 배포 설정",
       "html": "이 부록 막힘의 최다 원인은 두 가지입니다. ① 배포할 때 액세스 권한을 <b>‘모든 사용자’</b>로 안 바꾸고 기본값(나만)으로 둠 — 피코는 로그인을 못 하니 아무 오류 없이 조용히 실패합니다. ② 코드를 고친 뒤 <b>‘새 버전’ 재배포를 안 함</b> — 편집기에서 저장만 하면 옛 코드가 계속 돕니다. “시트에 안 쌓여요” 손이 올라오면 피코 코드를 보기 전에 <b>배포 관리 화면부터</b> 함께 여세요."},
    ]},
    {"title": "2단계 · 피코에 붙여넣고 실행", "items": [
      ...
      {"type": "callout", "kind": "info", "title": "왜 딱 60초마다 안 와요?",
       "html": "<code>INTERVAL</code>은 ‘<b>쉬는 시간</b>’만 정해요. 그런데 한 바퀴에는 <b>쉬는 시간 + 인터넷으로 보내는 시간</b>이 같이 들어가요. 인터넷 전송이 몇 초 더 걸리고 그 시간도 매번 조금씩 달라서, 실제 간격은 설정값보다 <b>살짝 길고 들쭉날쭉</b>합니다. ‘정확한 초’보다 ‘꾸준히 기록’이 목적이라 괜찮아요. (정확히 맞추고 싶으면 보낸 데 걸린 시간을 빼고 남은 만큼만 쉬게 하면 돼요.)"},
    ]},
