...
page("worldbank", "🌱", "나라별 CO₂·에너지", "환경·에너지·물리", "🌍 전 세계(국내 포함)",
  ...
  body='<div class="controls"><label>지표</label><select id="ind" onchange="run()">'
       ...
       '<div style="font-size:11.5px;color:#7a7f95;margin:6px 2px;text-align:center">가로=1인당 CO₂(톤) · 세로=재생에너지 비중(%) · 원 크기=인구 · <b style="color:#E0568A">🇰🇷 한국은 분홍 큰 원</b></div>'
       '<div class="qbox"><div class="qbox-t">🔎 탐구 질문</div><ul>'
...
async function run(){
  ...
  try{
    ...
    // 나라마다 최신 연도가 다르면 비교가 불공정 → 자료가 충분히 채워진 '최신 한 해'로 통일
    const cnt={};
    arr.forEach(r=>{ if(r.value!=null&&r.countryiso3code&&meta[r.countryiso3code]) cnt[r.date]=(cnt[r.date]||0)+1; });
    const yrs=Object.keys(cnt); const maxc=Math.max(...yrs.map(y=>cnt[y]));
    const refYear=yrs.filter(y=>cnt[y]>=0.7*maxc).sort().pop();
    const latest={};
